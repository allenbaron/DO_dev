# extract all mappings from ORDO ~SSSOM and compare with previous extraction
library(here)
library(tidyverse)
library(DO.utils)
...
ordo_path <- readline(prompt = "Input the local path to ORDO: ")

gs <- "1ziIO01RM-0NSfqAxGR5AjphnbAy5pIn9UnRTr3oYcZs/" # ORDO_SSSOM Google Sheet
gs_sheet <- "in_ORDO"


# Extract ORDO mappings (new version only) & compare with existing
ordo_version <- tools::file_path_sans_ext(basename(ordo_path))
ordo_out <- here::here("data/mapping", paste0(ordo_version, "-mappings.tsv"))
if (!file.exists(ordo_out)) {
    invisible(
        DO.utils::extract_ordo_mappings(
            ordo_path,
            as_skos = TRUE,
            output = ordo_out
        )
    )
    new_ORDO <- readr::read_tsv(ordo_out, col_types = "c") %>%
        DO.utils::tidy_sparql() #%>%
        dplyr::mutate(
            subject_source_version.new = ordo_version
        )
    current_status <- NULL
    withCallingHandlers(
        current_ORDO <- googlesheets4::read_sheet(gs, gs_sheet),
        gargle_error_request_failed = function(e) { current_status <- "error" }
    )

    if (!is.null(current_status)) {
        merged_ORDO <- dplyr::full_join(
            current_ORDO,
            new_ORDO,
            by = dplyr::join_by(subject_id, predicate_id, object_id)
        ) %>%
            dplyr::mutate(
                subject_source_version.old = subject_source_version,
                status = dplyr::case_when(
                    !is.na(subject_source_version.old) &
                        !is.na(subject_source_version.new) ~ subject_source_version.new,

                    subject_source_version,
                    subject_source_version.new
                )
            )
            )
            ### here
