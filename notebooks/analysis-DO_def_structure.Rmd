...
date: "Created: 2022-04-06<br style='line-height: 2;'>Updated: 2023-07-18"
output: 
    html_notebook:
        toc: true
        toc_float:
...
**ANSWER**
On 2022-04-04 Lynn stated "caused by" should generally be avoided.
...
gene_id
```
It seems ~ `r sum(gene_id$pct[c(1, 4:9)])` of all genetic has_material_basis_in statements have only the gene symbol (and not any type of name). While that's not a definitive majority, it is probably the predominant pattern, as the remaining ~ 50% is more variable.
...
Some typos... expected. There are few updates needed.
