...
I attempted to extract definitions from the MESH & NCI rdf files. Though the query works only NCI definitions are returned. It appears that there are _NO_ `meshv:scopeNote` triples for the MESH terms of interest.
```{r}
query_new_def <-
    '# Query non-deprecated DOIDs without definitions
    PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
...
Like my attempts to get definitions from the mesh.nt.gz file download, no meshv:scopeNote is available via the API, with or without inference on, for mesh:D010211 "Papilledema" which unquestionably has a "Scope Note" in the MESH search result (https://meshb.nlm.nih.gov/record/ui?ui=D010211).

This is the SPARQL query I submitted to the API:
```
