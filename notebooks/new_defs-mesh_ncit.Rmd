...
query_new_def <-
    ...
    WHERE {
        ...
        BIND(
            ...
            OPTIONAL {
                # ?uri meshv:scopeNote ?def .
                ?uri ?p ?o .
                ?uri meshv:identifier ?code .
            }
        }
        # LIMIT 100
        }
...
Like my attempts to get definitions with the mesh.nt.gz file download, no meshv:scopeNote is available via the API with or without inference on for mesh:D010211 "Papilledema" which unquestionably has a "Scope Note" in the MESH  search result (https://meshb.nlm.nih.gov/record/ui?ui=D010211).

The SPARQL query I submitted to the API
```
